...
<html>
    <head>
		<link rel="stylesheet" href="web.css" type="text/css">  
 	    <meta charset="ANSI">
        <title>Личная страница заказчика</title>
    </head>
    <body>
	...
	<tr>
		...
		<td align="center">
		<?php if ($Result["status"]==3):
			echo $Result["datavoz"]; else: echo "Не решён"; endif; 
		?>
		</td>
		<td>
		<?php if ($Result["status"]==3): ?>
		<form action="agreed.php" method="post" value="'.$idz.'"> 
		<input type="submit" name="button" value="Принять заказ"/>
		<input type="hidden" name="HF" value="<?php echo $idz?>"/> 				
		</form>
			<?php else: echo"Не готов"; endif;?>
		</td>
    </tr>
	<?php
		}
